...
void set_bg_opa(void* obj, int32_t v) {
    lv_obj_set_style_bg_opa(static_cast<lv_obj_t*>(obj), static_cast<lv_opa_t>(v), 0);
}
...
void splash_play_intro(lv_obj_t* root) {
    ...
    lv_obj_set_style_bg_opa(zap_line, 0, 0);
    ...
    start_anim(zap_line, set_bg_opa,    0,  255, 600, 400);
    start_anim(slogan,   set_text_opa,  0,  255, 800, 400);
}
